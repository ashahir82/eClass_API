...
if(isset($_GET['username'])) {
	...
	if (mysqli_num_rows($module) != 0) {
		...
		while ($rowModule = mysqli_fetch_assoc($module)) {
			$rowsResult = array();
			$result = mysqli_query($GLOBALS["con"],"SELECT * FROM `enroll` RIGHT JOIN `topic` ON `enroll`.`topic_id` = `topic`.`topic_id` WHERE `username` = '$username' AND `module_id` = " . $rowModule['module_id'] . " ORDER BY `topic`.`le_no`");
