...
if(isset($_POST['topic_id'])) {
	...
	//count note
	$notecount = mysqli_query($GLOBALS["con"],"SELECT * FROM `note` WHERE `topic_id` = $topic_id");
	$total_notes = mysqli_num_rows($notecount);
	if ($total_notes != 0) {
	...
	//count soalan
	$quizcount = mysqli_query($GLOBALS["con"],"SELECT * FROM `quiz` WHERE `topic_id` = $topic_id");
	$total_quizs = mysqli_num_rows($quizcount);
	if ($total_quizs != 0) {
		// output data of each row
